//using convex hull dp optimisation template from http://ideone.com/fAjoZu 
#include<set>
#include<cmath>
#include<iostream>
#include<algorithm>
...
int main()
{
	long long int n,q,x,y,z,qtype;
	ConvexHullDynamic A(true);
	cin>>n>>q;
	while(q--)
    {
    	cin>>qtype;
    	...
    	{
    		cin>>x>>y>>z;
    		in[y].intercept+=(in[y].speed-z)*x;
    		//cout<<"hey:"<<in[y].intercept<<endl;
    		in[y].speed=z;
    		in[y].time=x;
    		A.addLine(in[y].speed,in[y].intercept);
    	}
    	else
    	{
    		cin>>x;
    		cout<<A.getBest(x)<<endl;
    	}
    }
